//2020
/*
Write a program based on BFS which can answer questions of the type: "Find the a path from X to Y" 
Which should result in a list of vertices traversed from X to Y if there is a path.
*/
import edu.princeton.cs.algs4.In;     

public class Ugraph2 {
    ...
    public static class BreadthFirstSearch {
        ...
        public BreadthFirstSearch(Graph G, int s) {                       //tar argumneterna G-graf och s-source     
        ...
        //vi börjar med att enquea alla som är adjecent till vetex s. För att accessa den adjecenta måste vi dequea den först
        private void bfs(Graph G, int s) {
            Queue<Integer> queue = new Queue<Integer>();
            marked[s] = true;                                        //markera source 
            queue.enqueue(s);                                        //lägg source på queue 
                       
            while (!queue.isEmpty()) {                               //sålänge queue inte är empty 
                                                                     //tabort s från queue, och spara på variablen v
                int v = queue.dequeue();                             //tabort nästa vertice från queue för ta adjecenta verticer till den
                for (int w : G.adj(v)) {                             //spara alla adjecenta v i w
                    if (!marked[w]) {                                //sålänge den är omarkerad 
                        marked[w] = true;                            //markera den 
                        edgeTo[w] = v;                               //komm ihåg  edge verticen 
                        queue.enqueue(w);                            //om den inte är markerad lägg den på queue
 ...
 //definerar grafen med string, instead of index
    ...
    public static class SymbolGraph {
        ...
        public SymbolGraph(String filename, String sp) {                //har parametrarna för dbasen och städerna,  //filnamn=key ,
            ...
            // bygg grafen genom att rita upp pathen mha edge
    ...
    public static void main(String[] args) {
       ...
       int startLocation = syG.index("TN");                                     //begynnesle address
        ...
        BreadthFirstSearch bfs = new BreadthFirstSearch(G,startLocation);

        if (bfs.hasPathTo(finalDestination)) {                                  //om final destination vertexen är markerad
            System.out.print("Väg från " + syG.name(startLocation) + " till " + syG.name(finalDestination) + ":\n");
            for (int j : bfs.pathTo(finalDestination)) {                        //spara pathen vi hade på stack på variablen j
                if (j == startLocation) {
